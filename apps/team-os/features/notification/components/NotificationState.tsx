...
import { AlertCircle, BellOff, LoaderCircle, ShieldAlert } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";

export function NotificationLoadingState({ label = "正在读取消息数据…" }: { label?: string }) {
  return (
    <Card>
      <CardContent
  ...
  return (
    <Card className="border-dashed border-slate-300">
      <CardContent className="flex min-h-52 flex-col items-center justify-center p-6 text-center">
        <BellOff className="h-10 w-10 text-slate-300" aria-hidden="true" />
...
export function NotificationForbiddenState({ description }: { description: string }) {
  return (
