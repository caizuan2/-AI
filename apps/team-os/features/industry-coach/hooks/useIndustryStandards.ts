...
import { fetchIndustryStandards } from "@/apps/team-os/features/industry-coach/services/industry-coach-client";
import type { IndustryStandardsData } from "@/apps/team-os/features/industry-coach/types";

export function useIndustryStandards(initialCompanyId?: string) {
  const [data, setData] = React.useState<IndustryStandardsData | null>(null);
  ...
  const reload = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const nextData = await fetchIndustryStandards(selectedCompanyId);
      if (requestId === activeRequest.current) setData(nextData);
    } catch (caught) {
      if (requestId === activeRequest.current) setError(caught instanceof Error ? caught.message : "行业标准加载失败。");
    } finally {
