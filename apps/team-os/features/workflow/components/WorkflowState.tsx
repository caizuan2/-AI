...
import { AlertCircle, LoaderCircle, ShieldAlert, Workflow } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";

export function WorkflowLoadingState({ label = "正在读取自动化流程…" }: { label?: string }) {
  return (
    <Card>
...
export function WorkflowForbiddenState({ description }: { description: string }) {
  return (
    <Card className="border-amber-200 bg-amber-50/50">
      <CardContent className="flex min-h-52 flex-col items-center justify-center p-6 text-center">
        ...
        <p className="mt-4 font-semibold text-amber-950">当前角色不可管理工作流</p>
...
export function WorkflowEmptyState({
  ...
  return (
    <Card className="border-dashed border-slate-300">
      <CardContent className="flex min-h-52 flex-col items-center justify-center p-6 text-center">
        <Workflow className="h-10 w-10 text-slate-300" aria-hidden="true" />
