...
for command_name in chmod chown docker curl find flock install node readlink sed sha256sum stat; do
...
if [[ -z "$TARGET_RELEASE" ]]; then
  [[ -s "$PREVIOUS_RELEASE_FILE" ]] || die "no previous release is recorded; pass --target explicitly"
...
[[ $(calculate_orchestrator_sha256 "$TARGET_RELEASE") == "$ORCHESTRATOR_SHA256" ]] \
...
if [[ -n "$ORIGINAL_RELEASE" ]]; then
  ...
  [[ $(calculate_orchestrator_sha256 "$ORIGINAL_RELEASE") == "$ORIGINAL_ORCHESTRATOR_SHA256" ]] \
