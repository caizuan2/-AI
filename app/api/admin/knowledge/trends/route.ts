...
import {
  buildTrendRecommendations,
  summarizeKnowledgeTrends,
  type KnowledgeTrendReportItem
} from "@/lib/enterprise/knowledge-trend-report";
...
export async function GET(request: Request) {
  ...
  try {
    actor = await requireKbAdmin(request, {
      ...
      targetType: "admin_knowledge_trends"
    });
  } catch (error) {
  ...
  if (!hasDatabaseUrl()) {
    return apiError(databaseConfigError("分析知识长期趋势"));
  }
  ...
  try {
    ...
    const items: KnowledgeTrendReportItem[] = chunks.map((chunk) => {
      ...
      const trend = buildTrendDiagnosticsFromMetadata(chunk.metadata, {
    ...
    const summary = summarizeKnowledgeTrends(items);
    const lifecycleItems: KnowledgeLifecycleReportItem[] = chunks.map((chunk) => {
      const governance = readKnowledgeGovernanceMetadata(chunk.metadata);
      const scoped = scopeKeyFromMetadata(chunk.metadata, accessScope);
    ...
    const lifecycleSummary = summarizeKnowledgeLifecycle(lifecycleItems);
    const lifecycleByChunkId = new Map(lifecycleItems.map((item) => [item.chunkId, item.lifecycle]));
    const policyItems: KnowledgePolicyReportItem[] = chunks.map((chunk) => {
      const governance = readKnowledgeGovernanceMetadata(chunk.metadata);
      const scoped = scopeKeyFromMetadata(chunk.metadata, accessScope);
      const lifecycle = lifecycleByChunkId.get(chunk.id);
    ...
    return NextResponse.json({
      ...
      summary: {
        ...summary,
        lifecycle: {
          newCount: lifecycleSummary.newCount,
          growingCount: lifecycleSummary.growingCount,
          stableCount: lifecycleSummary.stableCount,
          decliningCount: lifecycleSummary.decliningCount,
          archiveCandidateCount: lifecycleSummary.archiveCandidateCount,
          unknownCount: lifecycleSummary.unknownCount
        },
        policy: {
      ...
      recommendations: [
        ...buildTrendRecommendations(items),
        ...buildLifecycleRecommendations(lifecycleItems),
        ...buildPolicyRecommendations(policyItems)
      ].slice(0, 120),
      diagnostics: {
        mode: "read_only_trend_analysis",
        dataCoreMode: coreState.diagnostics.mode,
        metadataPersisted: false,
        autoDeleteEnabled: false,
        autoMergeEnabled: false,
        shadowMode: summary.shadowModeCount > 0,
        scope: {
        ...
        sample: items.slice(0, 30).map((item) => ({
          ...
          trendScore: item.trend.trendScore,
          trendLabel: item.trend.trendLabel,
          trendConfidence: item.trend.confidence,
          staleRisk: item.trend.staleRisk,
          fastRising: item.trend.fastRising,
          staleHighScore: item.trend.staleHighScore,
          decliningTrend: item.trend.decliningTrend,
          evergreen: item.trend.evergreen,
          trendReason: item.trend.trendReason,
          shadowMode: item.trend.shadowMode,
          lifecycleStage: lifecycleByChunkId.get(item.chunkId)?.lifecycleStage ?? "unknown",
          lifecycleScore: lifecycleByChunkId.get(item.chunkId)?.lifecycleScore ?? 0.5,
          lifecycleSuggestion: lifecycleByChunkId.get(item.chunkId)?.lifecycleSuggestion ?? "等待更多使用数据",
